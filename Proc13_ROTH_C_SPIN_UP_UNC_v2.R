...
# 3) Vector must be an empty points vector. 

Vector<-vect("pointsFromQGIS.shp")


# 4) Stack_Set_1 is a stack that contains the spatial variables 

Stack_Set_1<- rast("Stack_Set_SPIN_UP_AOI.tif")

# 5) Create A vector to save the results

C_INPUT_EQ<-Vector

# use this only for backup

# C_INPUT_EQ<-readOGR("OUTPUTS/1_SPIN_UP/SPIN_UP_AOI.shp")

# 6) Extract variables to points
...
names(C_INPUT_EQ)[2]="SOC_FAO"
names(C_INPUT_EQ)[3]="Cinput_EQ"
names(C_INPUT_EQ)[4]="SOC_pedotransfer"
names(C_INPUT_EQ)[5]="DPM_pedotransfer"
names(C_INPUT_EQ)[6]="RPM_pedotransfer"
names(C_INPUT_EQ)[7]="BIO_pedotransfer"
names(C_INPUT_EQ)[8]="HUM_pedotransfer"
names(C_INPUT_EQ)[9]="IOM_pedotransfer"
names(C_INPUT_EQ)[10]="CIneq_min"
names(C_INPUT_EQ)[11]="CIneq_max"
names(C_INPUT_EQ)[12]="SOC_min"
names(C_INPUT_EQ)[13]="DPM_min"
names(C_INPUT_EQ)[14]="RPM_min"
names(C_INPUT_EQ)[15]="BIO_min"
names(C_INPUT_EQ)[16]="HUM_min"
names(C_INPUT_EQ)[17]="IOM_min"
names(C_INPUT_EQ)[18]="SOC_max"
names(C_INPUT_EQ)[19]="DPM_max"
names(C_INPUT_EQ)[20]="RPM_max"
names(C_INPUT_EQ)[21]="BIO_max"
names(C_INPUT_EQ)[22]="HUM_max"
names(C_INPUT_EQ)[23]="IOM_max"

# SAVE the Points (shapefile)

#setwd("C:/TRAINING_MATERIALS_GSOCseq_MAPS_12-11-2020/OUTPUTS/1_SPIN_UP")
writeVector(C_INPUT_EQ, "SPIN_UP_AOI.shp", overwrite=TRUE) 
